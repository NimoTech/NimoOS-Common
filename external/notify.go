...
const (
	NimoOSURLFilename = "nimoos.url"
	APINimoOSNotify   = "/v1/notify"
)
...
func (n *notifyService) SendNotify(path string, message interface{}) error {
	...
	url := strings.TrimSuffix(address, "/") + APINimoOSNotify + "/" + path
...
func NewNotifyService(runtimePath string) NotifyService {
	return &notifyService{
		addressFile: filepath.Join(runtimePath, NimoOSURLFilename),
	}
}
